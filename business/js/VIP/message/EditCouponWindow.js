Ext.define('VIP.message.EditCouponWindow', {
	...
	load : function(messageId) {
		...
		MessageAction.findCouponById(this.messageId, function(result) {			
			...
			if(me.down('#startDate').getValue()!=null){
				var date = new Date();
				date.setHours(0);
				date.setMinutes(0);
				date.setSeconds(0);
				date.setMilliseconds(0);
				if(me.down('#startDate').getValue().getTime()>=date.getTime()){
					me.down('#startDate').setMinValue(date);
					me.down('#startDate').setReadOnly(false);
				}else{
					me.down('#startDate').setMinValue(data.startDate);
					me.down('#startDate').setReadOnly(true);
				}
			}
				...
				items:[{
					...
					items: [{
							...
							if(panel.items.length>1){
								...
								if(values.id!=null&&values.id!=""){
									Ext.MessageBox.confirm('警告', '你确定删除该描述中的图片及内容吗？', function(btn) {
										if (btn == 'yes') {
											MessageAction.deleteCouponPictures(values.id,function(actionResult){
						...
						handler: function(field){
							...
							if(values.id!=null&&values.id!=""){
								MessageAction.updateCouponPictures(values.id,url,values.content,function(actionResult){
									if(actionResult.success){
										Ext.Msg.info('修改成功');
									}else{
										Ext.Msg.error(actionResult.message);
								...
								MessageAction.addCouponPictures(me.messageId,url,values.content,function(actionResult){
									if(actionResult.success){
										Ext.Msg.info('添加成功');
									}else{
										Ext.Msg.error(actionResult.message);
	...
	save : function(btn){
		...
		if (form.isValid()) {
			...
			MessageAction.editCoupon(values, function(actionResult) {
				if (actionResult.success) {
